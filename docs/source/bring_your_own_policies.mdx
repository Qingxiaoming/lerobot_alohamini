...
└── README.md                      # symlink → ../../../../docs/modules/policies/my_policy.md
```

Two notes:

- The `README.md` next to the source is a **symlink** into
  `docs/modules/policies/<name>.md` — the actual file lives under `docs/`. Existing policies (act,
  smolvla, diffusion, …) all do this; copy one of those symlinks. The policy README is conventionally
  minimal: paper link + BibTeX citation.
...
The general expectations are in the [contribution guide](./contributing) and the
[PR template](https://github.com/huggingface/lerobot/blob/main/.github/PULL_REQUEST_TEMPLATE.md). On
top of those, reviewers will look for:
...
- [ ] `src/lerobot/policies/<name>/README.md` symlinked into
      `docs/modules/policies/<name>.md`; user-facing `docs/source/<name>.mdx` written and added to
      `_toctree.yml`.
